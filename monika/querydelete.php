...
<html lang="bg">
  <head>
    <meta charset="UTF-8">
    <link rel="stylesheet" type="text/css" href="style/cpdel.css"/>
  ...
  <body>
    <?php
      ...
      $sql = "SELECT * FROM query WHERE deleted='1'";
      $result = mysqli_query($conn,$sql);
      echo('<h1 align="center">Изтрити заявки</h1>');
      ...
      if(mysqli_num_rows($result) > 0){
        while($row = mysqli_fetch_assoc($result))
        {
          ...
          echo('<a href="deleteforever.php?clientid='.$rowid.'" class="buttons"><i class="fas fa-trash-alt" style="background:red; float:right;"></i></a>');
        ...
        echo('<div class="showdivs">');
        echo('<h1>Няма изтрити заявки</h1>');
        echo('</div>');
      }
      echo('</div>');
      mysqli_close($conn);
